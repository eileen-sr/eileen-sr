...
fn writeOutputFileHeader(out: *Io.Writer) !void {
    try out.writeAll(
        ...
        \\            },
        \\        };
        \\        pub const key_field_desc: FieldDesc = .{ .number = 1, .encoding = .normal };
        \\        pub const value_field_desc: FieldDesc = .{ .number = 2, .encoding = .normal };
        \\
        \\        key: K,
        \\        value: V,
        \\    };
...
const FieldType = union(enum) {
    ...
    pub fn encoding(ft: FieldType) Encoding {
        const name = switch (ft) {
            .map => return .normal,
            inline else => |t| t,
        };
...
fn compileMessage(mode: CompilationMode, arena: Allocator, l: *Lexer, out: *Io.Writer) !void {
        ...
        if (output_structures)
            try out.print("    {s}: {f} = {s},\n", .{ field_name, field_type, switch (field_type) {
                .regular => |regular| if (std.meta.stringToEnum(FieldType.Primitive, regular)) |p| switch (p) {
                    .int32, .int64, .sint32, .sint64, .uint32, .uint64, .float, .double => "0",
                    .string, .bytes => "\"\"",
                    .bool => "false",
                } else "null",
                .repeated, .map => ".empty",
    ...
    if (output_descriptors) {
        for (field_descs.items) |desc| {
            try out.print(
                "    pub const {s}_field_desc: FieldDesc = .{{ .number = {s}, .encoding = .{t} }};\n",
                .{ desc.name, desc.number, desc.encoding },
            );
        }
    }
